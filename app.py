...
#mytdb=mysql.connector.connect(host='localhost',user='root',password='jyothsna',db='snmproject')
Session(app)
mydb=connection.MySQLConnection(user='root',password='jyothsna',host='localhost',database='snmproject')
@app.route('/')
def home():
...
def otp(enudata):
    if request.method=='POST':
        ...
        try: 
            dudata=decode(data=enudata) #udata={'username':username,'uemail':uemail,'password':password,'otp':gotp}
            print(dudata)
    ...
    if not session.get('user'):
        if request.method=='POST':
            ...
            if bdata[0]==1:
                ...
                bpassword=cursor.fetchone() #(363636000000000000)
        ...
        try:
            if request.method=='POST':
                ...
                if (pattern.match(sdata)):
                    ...
                    cursor.close()
                    return render_template('dash.html',sdata=sdata)
                else:
                    flash('no data found')
                    return redirect(url_for('dashboard'))
            else:
